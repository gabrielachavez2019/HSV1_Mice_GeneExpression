...
#
# Script Name: RNA-seq analysis post mapping with TOPHAT2 and CUFFLINKS
#
...
library(cummeRbund)
setwd("/Users/joneslab/Documents/Analysis/HSV1-mouse")

cuff_data <- readCufflinks('diff_out_test')
...
write.table(diffGenesOutput, 'diff_genes_mix.txt', sep='\t',row.names = F, col.names = T, quote = F)
...
sp<-csScatter(genes(cuff_data),"Male_dLAT","Male_U1",smooth=T)
sp
...
dend<-csDendro(genes(cuff_data))  #To build a dendogram
dend

m<-MAplot(genes(cuff_data),"Male_dLAT","Male_U1")
m

mCount<-MAplot(genes(cuff_data),"Male_dLAT","Male_U1",useCount=T)
mCount
...
s<-csScatter(myGenes, "Male_dLAT","Male_U1",smooth=T)
s

v<-csVolcano(myGenes, "Male_dLAT","Male_U1")
v
############
# Analysis of specific gene candidates, I sued my awk script before but his one is more efficient
############
jones_list <- read.csv("PMC187408.csv", TRUE, ",")
myGeneIds <- (jones_list$Selection2)
...
myGene<-getGene(cuff_data,"Stat2")
igb<-expressionBarplot(isoforms(myGene),replicates=T)
igb
